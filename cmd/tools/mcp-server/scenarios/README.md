...
source cmd/tools/mcp-server/scenarios/common.sh

# 3. Run setup (injects the failure)
bash cmd/tools/mcp-server/scenarios/operator-crash-setup.sh

# 4. Call classify_failure and compare to ground truth
call_mcp_tool classify_failure '{}' | jq .
cat cmd/tools/mcp-server/scenarios/operator-crash-ground-truth.json | jq .expected_classification

# 5. Teardown (restores the cluster)
bash cmd/tools/mcp-server/scenarios/operator-crash-teardown.sh
```
